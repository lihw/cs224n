...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        embeddings = tf.Variable(self.pretrained_embeddings, "embeddings")
        embed = tf.nn.embedding_lookup(embeddings, self.input_placeholder)
        _, __, embedding_size = tf.shape(embed)
        embeddings = tf.reshape(embed, shape = (None, self.config.n_features * embedding_size))
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        W = initializer(shape = (self.config.n_features, self.config.hidden_size))
        U = initializer(shape = (self.config.hidden_size, self.n_classes))
    ...
    def add_loss_op(self, pred):
        ...
        """
        ### YOUR CODE HER
        h = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = pred, 
                labels = self.lables_placeholder))
        ### END YOUR CODE
        return loss
